<template>
  <div class="mycar">
    <mt-header title="我的购物车" class=" bg-my pt-1">
      <div @click="back()" slot="left">
        <img src="img/pro_detail_icon/back.png" alt="" style="width: 30%;" />
      </div>
      <router-link :to="`/myLike/${uid}`" slot="right">
        收藏
      </router-link>
    </mt-header>
...
export default {
  ...
  methods: {
     del(pid){
        this.axios.get(`/product/dropshoppingcart?uid=${this.uid}&pid=${pid}`).then((result) => {
    ...
    getcart(){
      ...
      this.axios
      .get(`/product/getshoppingcart?uid=${this.uid}`)
      .then((result) => {
